 ...
 *
 * SECURITY: X-Forwarded-For is "client, proxy1, proxy2, ...". A malicious
 * client can set arbitrary leftmost values, so the FIRST entry is spoofable.
 * Behind a single trusted reverse proxy (Render), the real client IP is the
 * LAST entry — the one our proxy appended. Taking the rightmost value prevents
 * trivial rate-limit bypass via a forged X-Forwarded-For header.
 */
